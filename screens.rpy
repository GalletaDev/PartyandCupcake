...
style button:
    ...
    outlines [(absolute(2), "#ffffff", absolute(2), absolute(0))]

style button_text is gui_text:
    properties gui.text_properties("button")
    yalign 0.5
    outlines [(absolute(2), "#ffffff", absolute(1), absolute(0))]
...
define textbox_decoration = Transform("gui/textbox.png", xalign=0.5, yalign=1.0, xsize=1380, ysize=250)
    ...
    vbox:
        for i in items:
            textbutton i.caption:
                hover_sound kick_sound
                activate_sound ok_sound
                action i.action
...
screen quick_menu():
    ...
    if quick_menu:
        add "#000" at Transform(xsize=750, ysize=60, xalign=0.5)
        hbox:
            style_prefix "quick"
    ...
    frame:
        padding(10, 10)
        align(.9, .9)
        text "[persistent.chapter_actual]"
    ...
    hbox:
        ...
        frame:
            ...
            hbox:
                ...
                if main_menu:
                    if chapter.get_quantity() <= 120:
                        textbutton _("Repetir"):
                            hover_sound kick_sound 
                            activate_sound ok_sound
                        ...
                        textbutton _("Comenzar"):
                            hover_sound kick_sound 
                            activate_sound ok_sound
                            action Start(label='starts_end_game')

                if persistent.complex_game and main_menu:
                    textbutton _("Reiniciar"): 
                        hover_sound kick_sound 
                        activate_sound ok_sound
                        action Show("confirm", 
                            message=_("Reiniciaras el tiempo del juego de nuevo\n esto no afectara tus logros \n ¿estas seguro?"),
                            yes_action=[Notify(_("Se a reiniciado los capitlos correctamente")), Function(reset_game_full), Hide()],
                            no_action=[Hide()])
...
define logo_img = "gui/window_icon.png"
define main_menu_ = "gui/main_menu.png"
define dorkis_main = "gui/dorkis.png"
define choko_main = "gui/chokis.png"


transform move_animation_main_menu_1():
    subpixel True
    xpos -300
    parallel:
        linear 2 xoffset 3 yoffset 2
        linear 1 xoffset -2 yoffset 1
        linear 2 xoffset 1 yoffset 1
        linear 2 xoffset 2 yoffset -1
        linear 1 xoffset -1 yoffset -2
        repeat


transform move_animation_main_menu_2:
    subpixel True
    xpos -300
    parallel:
        linear 1 xoffset 1 yoffset 2
        linear 2 xoffset -2 yoffset 1
        linear 2 xoffset 1 yoffset 2
        linear 1 xoffset 1 yoffset -1
        linear 2 xoffset -1 yoffset -2
        repeat

transform sacudic_public:
    subpixel True
    parallel:
        linear 1 xoffset 1 yoffset 2
        linear 1 xoffset -2 yoffset 1
...
transform alpha_animation_main:
    subpixel True
    alpha 0.4
    block:
        linear 8 alpha 0.4
...
screen notify_web_game:

    frame:
        padding(25, 25)
        align(0.98, 0.08)

        add "gui/public_ls.png" at Transform(xalign=0.5, yalign=0.5, size=(512, 256))

    textbutton _("!Juega ahora¡") at sacudic_public:
        xalign 0.85
        yalign 0.3
        action OpenURL("https://galletadev.itch.io/little-secrets-io")
...
screen main_menu():
    ...
    # add main_menu_
    add "remoline_infinity" at color_wipe_anim
    add choko_main at move_animation_main_menu_2
    add dorkis_main at move_animation_main_menu_1
    add "gui/effect_pink.png" at alpha_animation_main
    add "particules_white_blosson" at Transform(yoffset=-450 ,rotate=-180)
    add logo_img at Transform(xalign=1.0, yalign=1.0, size=(512, 512))
    ...
    if persistent.complex_game:
        add "gui/start.png" at Transform(xalign=0.04, yalign=0.85, zoom=0.25)
...
screen game_menu(title, scroll=None, yinitial=0.0, spacing=0, identify=0):

    style_prefix "game_menu"

    if main_menu and identify == 0:
        add gui.main_menu_background
        use navigation
    ...
    if not main_menu:
        add None
        use navigation_2
    elif main_menu and not identify == list([1, 2, 3, 4]):
        use navigation_2
...
screen about():
    ...
    use game_menu(_("Acerca de"), scroll="viewport", identify=1):
...
screen save():
    ...
    text _("Esto no esta disponible en este juego"):
        align(.5, .5)
...
screen preferences():
    ...
    use game_menu(_("Opciones"), scroll="viewport", identify=2):
...
screen history():
    ...
    use game_menu(_("Historial"), scroll=("vpgrid" if gui.history_height else "viewport"), yinitial=1.0, spacing=gui.history_spacing, identify=3):
